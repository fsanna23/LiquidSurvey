...
//Importazione del JSON come Object
import jsonData from '../JsonQuestionnaire.json';
...
function JsonLoader() {
						...
						className={classes.wrapper}>
							...
							<Grid className={classes.grid} container spacing={3}>
								...
								{question.data.images.map((img) => 
									...
									<Paper variant="outlined" className={classes.imagePaperContainer}>
										{loadImage(img)}
									</Paper>
								)}
								</Grid>
							...
							<Box align="left" className={classes.elementContainer}>{question.data.description}</Box>
			 				...
			 				{question.data.choices.map((s) =>
								...
								<Box align="left" className={classes.choicesContainer}>
										...
										<FormControlLabel value={s.value} control={<Radio color="primary"/>} label={s.value} />
		...
		return(
			...
			<div>
				...
				<DragDropContext onDragEnd={(param)=>{
					...
					const [reorderList] = question.data.choices.splice(srcIndex, 1); //si rimuove l'indice sorgente dal nuovo array
				...
				{(provided, _) => (
					...
					<Draggable key={item.id} index={i} draggableId={'draggable-'+item.id}>
						{(provided, snapshot) => (

							<Card className={classes.dragndropItem} ref={provided.innerRef} {...provided.dragHandleProps} {...provided.draggableProps}>
								<CardContent>
								<DragHandleIcon className={classes.dragHandleIcon} />
								<div className={classes.rankingCardContent}>{item.value}</div>
								</CardContent>
							</Card>
						)}
					</Draggable>
				...
				<Typography component={'span'} color="textPrimary" align="center">
					<Grid>
						...
						className={classes.wrapper}>
							...
							<Box align="left" fontWeight="fontWeightBold" className={classes.elementContainer}>{question.data.description}</Box>			 				
				 					...
				 					<FormControl component="fieldset">
			 							...
			 							{	
			 								...
			 								[ ...Array((Number(question.data.maxValue) + 1) - Number(question.data.minValue))].map((s, i) =>
				 								...
				 								control={<Radio checked={linearScaleSelectedValue == Number(question.data.minValue) + i} color="primary" />}
	...
	const loadImage = (image) => {
		...
		return(
			<img className={classes.imageFormat} src={path} />
		);
			...
			<Typography component={'span'} color="textPrimary" align="center">
					<Grid>
						...
						className={classes.wrapper}>
							...
							<Box align="left" className={classes.elementContainer}>{question.data.description}</Box>
							<div className={classes.singleImageContainer} >{loadImage(question.data.img)}</div>
						</Paper>
					</Grid>
				</Typography>
		);
		...
		<div>
			...
			{jsonData.map((item) =>
				...
				<div>	
					...
					<Box display="flex" justifyContent="flex-end" className={classes.pagesSwitchButtonContainer}>
					{/*If currentPage is > 0 it shows the "Back" button in order to scroll back one page*/}
					{currentPage > 0 ?
						<Button className={classes.pagesSwitchButton} variant="contained" onClick={handlePaging(0)} color="primary">
						Back
						</Button> : ""
					}
					...
					{currentPage === (item.pages.length - 1) ?
						<Button className={classes.pagesSwitchButton} variant="contained" /*GESTIRE ONCLICK*/ color="primary">
						Save
						</Button>
						:
						<Button className={classes.pagesSwitchButton} variant="contained" onClick={handlePaging(1)} color="primary">
						Next
						</Button>
					}
					</Box>
